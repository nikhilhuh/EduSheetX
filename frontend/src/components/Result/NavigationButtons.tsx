...
const NavigationButtons: React.FC = () => {
  ...
  return (
    <div className="flex flex-wrap justify-between gap-4 items-centern text-xs tablet:text-sm">
      <Link title="Go back to Home" to="/" className="inline-block">
        <button className="flex items-center gap-2 bg-blue-50 text-blue-600 cursor-pointer font-medium px-5 py-2 rounded-full shadow hover:bg-blue-200 transition">
          <Home className="h-3 w-3 tablet:w-5 tablet:h-5" />
          <span>Back to Home</span>
        </button>
      </Link>
      <Link title="Give Another Test" to={`/subjects/${subjectName}/${topicName}`} className="inline-block">
        <button className="flex items-center gap-2 bg-blue-50 text-blue-600 cursor-pointer font-medium px-5 py-2 rounded-full shadow hover:bg-blue-200 transition flex-wrap">
          <ImLoop className="h-3 w-3 tablet:w-5 tablet:h-5" />
